...
def search_known_issues(query, es):
    ...
    if cached_result:
        ...
        return result, True, elapsed_time  # Ensure 3 values are returned (result, cache hit, time)
    ...
    search_query = {
        "query": {
            "bool": {
                ...
                "minimum_should_match": 1  # At least one condition must match (either semantic or vector search)
            }
        }
    }
    ...
    try:
        ...
        return {"msg": "Error during Elasticsearch query"}, 500  # return error message
    ...
    return result, False, elapsed_time  # Return 3 values: result, cache hit, elapsed time
    ...
    **Solution**: [Provide the best possible solution based on the context] else respond with "No feasible solution found".
    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            model=model,
            messages=[{"role": "system", "content": "You are a problem-solving assistant. Help find the best solution based on the context."},
        ...
        solution = parse_solution_response(response['choices'][0]['message']['content'])
...
def parse_solution_response(response_text):
    try:
        ...
        solution = next((line.split(":")[1].strip() for line in lines if line.startswith("**Solution**")), "No feasible solution found")
        return solution
    except Exception as e:
